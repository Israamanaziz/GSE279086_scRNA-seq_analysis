...
```{r Read all of the individual seurat objects}
...
```{r sanity check for duplicate genes in samples}
sapply(seurat_list, function(obj) {
  sum(duplicated(rownames(GetAssayData(obj, layer = "counts"))))
})

# There were no duplicate genes in any of the sample, therefore there is no need to run and save clean seurat list.
...
```{r Dimension reduction plot on PCA}
