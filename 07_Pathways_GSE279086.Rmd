...
# ---- Extract top pathways for each cell type and combine ----
top_summary_df <- all_pathways_df %>%
  group_by(CellType) %>%
  arrange(desc(abs(NES)), .by_group = TRUE) %>%
  slice_head(n = 10) %>%
  ungroup()

# ---- Save the summarized top pathways to a single CSV ----
summary_csv_path <- file.path(deg_path, "All_CellTypes_Top10_Reactome_GSEA_Summary.csv")

write.csv(top_summary_df, summary_csv_path, row.names = FALSE)

message("Summarized top pathways CSV successfully written to: ", summary_csv_path)
